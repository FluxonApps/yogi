...
use crate::{fork, Archive, BehaviorDescriptor, BeingId, Lineage};
...
/// Run `iterations` of MAP-Elites illumination over `archive`.
///
/// If the archive is empty it first seeds it with `seed_genome` as the `founder_id` founder. Each
/// iteration then: samples a parent elite uniformly (*branch from any ancestor*), forks a child
/// (inheriting the genome verbatim, ancestry recorded), applies the variator's closed-surface
/// mutations, evaluates the child, maps its behavior to a cell via `descriptor`, and offers it to the
/// archive (kept iff it beats that cell's elite). Deterministic given `seed`.
...
pub fn illuminate(
    ...
    for _ in 0..iterations {
        // Snapshot the parent (clone out so we can mutate the archive afterwards). Uniform over the
        // current elites = branch from any ancestor; empty archive can't happen post-seed.
        ...
        let (parent_lineage, parent_genome) = &parents[rng.below(parents.len())];

        let child = fork(parent_lineage, parent_genome, next_id);
        next_id += 1;
