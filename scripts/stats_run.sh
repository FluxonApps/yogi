#!/usr/bin/env bash
# STATS (F1 credibility): operator ⊕=3a+2b ratchet at 3 seeds × n=80 unseen-operand held-out → mean±std.
# Turns the headline "0→8/8 (n=8, 1 seed)" into a defensible number. Real qwen3:8b, free verifier.
set -uo pipefail; cd "$(dirname "$0")/.." || exit 1
echo "skill=⊕(3a+2b)  held-out n=80 (unseen operands 9-12)  seeds=1,2,3"
counts=""
for s in 1 2 3; do
  o=$(STUDENT=mlx-community/Qwen3-8B-4bit GOAL_KIND=numeric BIG_HELDOUT=1 SEED=$s THINK_OFF=1 BATCH=2 \
