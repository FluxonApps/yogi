...
b2_before=sum((safe_run(ask(m0,t0,c2(a,b),300))==str(g(a,b))) for a,b in test)   # cold ⊠ before (~0, novel)
b1_before=sum(parse(ask(m0,t0,c1(a,b),400))==f(a,b) for a,b in test)   # ⊞ retention baseline (~6/8 from F8)
...
# HEAVY ⊞ replay (the F3 fix): self-gen ⊞ CoT traces for many pairs to retain the prior abstraction
taught1=lambda a,b:f"The operator ⊞ is defined by a ⊞ b = 5a+3b+7. What is {a} ⊞ {b}? Show your working, then give the integer.{NT}"
rep=0; RP=[(a,b) for a in range(2,9) for b in range(2,9)]; random.shuffle(RP)
for a,b in RP:
    if rep>=30: break
    r=ask(m0,t0,taught1(a,b),300)
    if parse(r)==f(a,b): rep+=1; rows.append({"prompt":c1(a,b),"completion":" "+strip(r).strip()})
print(f"heavy ⊞ replay (F3 fix): {rep} ⊞ CoT traces mixed with 40 ⊠ traces",flush=True)
...
print(f"self-gen ⊠ via PROGRAM lever: {yok}/40 verified (high yield => ⊞ reused to acquire ⊠)",flush=True)
del m0,t0
...
print(f"\n=== PHASE 2b-v3 RESULT (recursion + F3 heavy-replay fix) ===",flush=True)
print(f"  ⊠ tool-use (plain prompt->program): cold before {b2_before}/8 -> after {b2_after}/8",flush=True)
