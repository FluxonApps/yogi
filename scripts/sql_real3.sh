#!/usr/bin/env bash
# BIRD usability — v3 (REACHABLE-floor regime): FASTER (compact schema, validated 1.47x + ~5x shorter LoRA seqs; --max-seq-length)
...
BIRD=/tmp/yogi_bird/minidev/MINIDEV; W=/tmp/yogi_sqlreal3; mkdir -p "$W/data"; PHASE="${PHASE:-probe}"; NSAMP="${NSAMP:-6}"
...
items=[q for q in Q if q['db_id'] in use and q['difficulty'] in ('simple','moderate')]; random.seed(0); random.shuffle(items)
train=items[:48]; test=items[48:72]
print(f'REACHABLE regime: simple+moderate only (excluding challenging/beyond-reach), {len(items)} available',flush=True)
