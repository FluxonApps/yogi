...
op=lambda a,b: 3*a+2*b+7
prompt=lambda a,b: f"Define x ⊕ y = 3x + 2y + 7. Compute {a} ⊕ {b}. End with 'Answer: N'. /no_think"
def ans(o):
    o=o.split('</think>')[-1]; m=re.findall(r'[Aa]nswer\s*:?\s*(-?\d+)',o) or re.findall(r'(-?\d+)',o); return int(m[-1]) if m else None
random.seed(0)
train=[(random.randint(1,9),random.randint(1,9)) for _ in range(NTR*2)]; train=list(dict.fromkeys(train))[:NTR]
heldp=[(random.randint(11,19),random.randint(11,19)) for _ in range(NTE*3)]; heldp=list(dict.fromkeys(heldp))[:NTE]
...
def evalheld(adapter):
    ...
    for a,b in heldp:
        o=generate(m,t,prompt=t.apply_chat_template([{"role":"user","content":prompt(a,b)}],add_generation_prompt=True,tokenize=False),max_tokens=120,verbose=False)
...
for rnd in range(1,R+1):
    ...
    for a,b in train:
        for _ in range(K):
            o=generate(m,t,prompt=t.apply_chat_template([{"role":"user","content":prompt(a,b)}],add_generation_prompt=True,tokenize=False),max_tokens=120,verbose=False,sampler=samp)
            if ans(o)==op(a,b):
                kk=(a,b)
    ...
    r=subprocess.run([sys.executable,"-m","mlx_lm.lora","--model",STU,"--train","--data",d,"--batch-size","1",
      "--num-layers","8","--iters",str(it),"--learning-rate","1e-4","--max-seq-length","256","--grad-checkpoint","--adapter-path",ad],capture_output=True,text=True)
