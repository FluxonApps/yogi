#!/usr/bin/env bash
# CEILING PUSH — plan-then-solve DECOMPOSITION on top of the confirmed interactive tools. The remaining ~52%
# errors are hard COMPOSITION queries (multi-join, nested aggregation) the 8B mis-structures. Here it first
# EXPLORES (FKs + sample VALUES), then writes an explicit PLAN (tables, FK joins, filters w/ verified values,
# aggregation/grouping), then writes SQL following the plan, then run/fix. DIN-SQL-style structure layered on
# the 48% tool base. items[0:80], compare to base 39/80 (48%). Zero salary.
...
def agent(q):
    ...
    # PLAN step (decomposition): structured outline before SQL
    plan=gen(info+"\nWrite a short PLAN for the query as bullet points:\n- tables needed\n- joins (use the foreign keys above)\n- filters (use exact verified values)\n- aggregation/grouping/order/limit\nOutput only the plan. /no_think",220).split('</think>')[-1].strip()
    base=info+f"\nPlan:\n{plan}\nNow write ONE SQLite SELECT query following the plan. Output only the SQL. /no_think"
    ...
    for _ in range(T-1):
        ...
        sql=exsql(gen(base+f"\nYour query:\n{sql}\nResult — {fb}\nIf wrong/errored correct it (follow the plan, FKs, verified values). Else repeat. Only SQL. /no_think")); rows,err=run(db,sql); final=sql
...
for i,q in enumerate(test):
    ...
    if (i+1)%20==0: print(f"  {i+1}/{len(test)}: decompose+tools {ok}",flush=True)
n=len(test)
print(f"\n=== DECOMPOSE + TOOLS — plan-then-solve on interactive base (items[{A}:{B}], n={n}, zero salary) ===",flush=True)
print(f"  decompose+tools {ok}/{n} ({100*ok//n}%)   vs  base tools 39/80 (48%)  one-shot 30/80 (37%)",flush=True)
print(f"  PUSHES CEILING ✓ iff > 39/80 — explicit planning fixes the hard composition (multi-join/nested) queries.",flush=True)
PY
