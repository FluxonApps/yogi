...
**Full reduction (agent-loop = correlated verified resampling).** Sequential retry is a CORRELATED, weaker sampler than independent pass@k: MBPP retry@8 reaches only +2 of its +12 reachable headroom (the model repeats similar errors on a hard item), whereas verifier-selected best-of-8 = pass@8 = +12. SQL is the easy regime (retry@2 +11 ~= its spread). So: reachable headroom (independent pass@k) is the ceiling; cheap sequential retry realizes it only where per-sample success is high; for low-probability reachable residuals use verifier-selected best-of-N instead. Deployment: measure pass@k; if retry@2 saturates well below it, switch to best-of-N (same verifier). Feedback content is ~irrelevant throughout.*
